...
export function testEncoder2() {
	...
	function hexToInt16(hex: string) {
		const bytes = [];
		for (let c = 0; c < hex.length; c += 4) {
			bytes.push(parseInt(hex.substr(c, 4), 16));
		}
		return bytes;
	}

	// Convert a byte array to a hex string
	function int16ToHex(bytes: Uint16Array) {
		const hex = [];
		let current: number;
		for (let i = 0; i < bytes.length; i++) {
			current = bytes[i] < 0 ? bytes[i] + 256 : bytes[i];
