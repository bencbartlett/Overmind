...
export class roleMiner extends Role {
    ...
    create(spawn, {assignment, workRoom = null, patternRepetitionLimit = 3}: creepCall): protoCreep {
        if (!workRoom) {
    ...
    depositContainer(creep) {
        ...
        }), (container: StructureContainer) => container.store[RESOURCE_ENERGY])[0];
        if (target) {
    ...
    depositLink(creep) {
        ...
        }), (link: StructureLink) => link.energy)[0];
        if (target) {
            return creep.assign(tasks('deposit'), target);
        }
    }
