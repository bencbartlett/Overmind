...
export class TerminalNetworkV2 {
	...
	private getColonyState(colony: Colony, resource: ResourceConstant): TN_STATE {
		const surplus = this.getSurplusThreshold(colony, resource);
		const target = this.getTargetThreshold(colony, resource);
		const tolerance = this.getTolerance(colony, resource);
		...
		if ((surplus != undefined && amount > surplus) ||
			(amount > target + tolerance && this.getRemainingSpace(colony) < MIN_COLONY_SPACE)) {
	 ...
	 */
	requestResource(requestor: Colony, resourceType: ResourceConstant, amount: number, tolerance = 0): void {
		...
		if (requestor.assets[resourceType] >= amount) {
			log.error(`TerminalNetwork.requestResource() called for ${requestor.print} requesting ${amount} of ` +
					  `${resourceType}, but colony already has ${requestor.assets[resourceType]} amount!`);
			return;
		}
		if (!this.colonyThresholds[requestor.name][resourceType]) {
			this.colonyThresholds[requestor.name][resourceType] = {
				target   : amount,
				surplus  : undefined,
				tolerance: tolerance,
			};
	...
	// /**
	//  * Sell excess minerals on the market TODO: rewrite this
	//  */
	...
	private handleRequestors(requestors: { [resource: string]: Colony[] },
		...
		for (const resource of RESOURCE_EXCHANGE_PRIORITIES) {
			for (const colony of (requestors[resource] || [])) {
				...
				requestAmount = minMax(requestAmount, TERMINAL_MIN_SEND, maxAmount);
	...
	private handleRequestInstance(colony: Colony, resource: ResourceConstant, requestAmount: number,
		...
		if (allowDivvying) {
			...
			const validPartners: Colony[] = _.take(
				_.sortBy(
			...
			for (const partner of validPartners) {
				if (remainingAmount < TERMINAL_MIN_SEND) {
					return;
				}
				const sender = partner.terminal!;
				const receiver = colony.terminal!;
				const amountPartnerCanSend = partner.assets[resource] - this.getTargetThreshold(partner, resource);
				const sendAmount = minMax(amountPartnerCanSend, TERMINAL_MIN_SEND, remainingAmount);
				// Send the resources or mark the terminal as overloaded for this tick
				if (sender.isReady) {
					this.transfer(sender, receiver, resource, sendAmount, `request instance for ${resource}`);
		...
		// TODO: add market integration for buying stuff you need
	}
	...
	private handleProviders(providers: { [resource: string]: Colony[] },
		...
		for (const resource of RESOURCE_EXCHANGE_PRIORITIES) {
			for (const colony of (providers[resource] || [])) {
				// Skip if the terminal is not ready
				...
				const maxAmount = resource == RESOURCE_ENERGY ? TerminalNetworkV2.settings.maxEnergySendSize
															  : TerminalNetworkV2.settings.maxMineralSendSize;
				sendAmount = minMax(sendAmount, TERMINAL_MIN_SEND, maxAmount);
	...
	run(): void {
		...
		this.handleProviders(this.activeProviders, [
			...
			this.passiveRequestors,
			this.equilibriumNodes,
			// this.passiveProviders // maybe I should add this? // edit: probably not
		]);
		...
		// TODO: handle buying energy if you have enough $$$ for it and need it
